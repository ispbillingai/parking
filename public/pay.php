...
<title><?= htmlspecialchars(I18n::t('pay_title')) ?></title>
<style>
  :root{
    --bg:#0b1020;
    --bg-2:#0f1530;
...
<div class="container">
  <div class="header">
    <h1><?= htmlspecialchars(I18n::t('pay_title')) ?></h1>
...
let session = null, autoResetHandle = null;

const $ = id => document.getElementById(id);
const fmt = c => (c / 100).toFixed(2);
const money = c => CFG.currency_symbol + ' ' + fmt(c);
const show = id => ['s1','s4'].forEach(s => $(s).classList.toggle('hidden', s !== id));
...
function renderUnpaid(data) {
  $('statusIcon').className = 'fail-icon';
  $('statusIcon').innerHTML = '&#10007;';
  $('s4Title').className = 'err';
  $('s4Title').textContent = CFG.i18n.not_paid;
  $('rcptEntry').textContent     = data.entered_at_human;
  $('rcptDuration').textContent  = data.duration_human;
  $('rcptAmountLabel').textContent = CFG.i18n.due;
  $('rcptAmount').textContent    = money(data.amount_cents || 0);
  $('s4Tip').classList.add('hidden');
  $('ttlBadge').classList.add('hidden');
}
...
$('lookup').onclick = async () => {
  ...
  try {
    ...
    session = data;
    if (data.already_paid) renderPaid(data); else renderUnpaid(data);
    show('s4');
